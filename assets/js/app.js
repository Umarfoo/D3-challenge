...
function makeResponsive(){
    ...
    // Removing old SVG Area for responsive to work
    d3.select("svg").remove();

    // Defining SVG Area
    var svgWidth = d3.select('.row').node().getBoundingClientRect().width;//window.innerWidth - 100;
    ...
    .attr("transform",`translate(${(width / 2) - 10}, ${height + margin.top + 20})`)
    .attr("class", "aText")
    .attr("class", "active")
    .attr("id", "poverty")
    .text("Poverty (%)")
    ...
    chartGroup.append("text")
    .attr("transform",`translate(${(width / 2) - 10}, ${height + margin.top + 40})`)
    .attr("class", "aText")
    .attr("class", "inactive")
    .attr("id", "age")
    .text("Age (Median)")
    ...
    chartGroup.append("text")
    .attr("transform",`translate(${(width / 2) - 10}, ${height + margin.top + 60})`)
    .attr("class", "aText")
    .attr("class", "inactive")
    .attr("id", "income")
...
// Initiating make responsive function
makeResponsive()

// Creating responsive graph onces window is resized.
